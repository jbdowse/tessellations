...
tessellations.build.demo1.animation = function buildDemo1Animation() {
	...
	var buildComponent = function buildComponent() {
		t.load.geom().animation();

		t.initializeDemos();

		t.build.demo1.points().shapes().patterns().styles();
		...
		a.caption("You can create different tile units with a single irregular shape by applying different color schemes, flipping, or rotating the shapes.").wait(1).hide('initPat', 1.5).rewind(1).to('sq4init', 'transform', shiftTo(pt.sq4(0), 0), 1).show('sq2init', 1).wait(0.5).show('mirrorLine', 0.5).wait(0.5).show(['sq2flip', 'sq4flip'], 1).wait(0.5).hide('mirrorLine', 1);

		a.caption("From these four units based on the same shape, you can create dozens of tessellations using a range of replication methods...").wait(3).to('sq4init', 'transform', shiftTo(pt.sq3(0), 0), 1).rewind(1).to('sq2init', 'transform', shiftTo(pt.sq3(1), 0), 1).rewind(1).to('sq2flip', 'transform', shiftTo(pt.sq3(2), 0), 1).rewind(0.75).hide('sq4flip', 1).wait(1);
		...
		a.caption("All this from a single irregular shape!").on('zoom').to('zoom', ['opacity', 'transform'], [1, scaleStr(pt.zoomLarge(), pt.zoomLarge())],
